...
namespace Lucinq.Sitecore.UnitTests.IntegrationTests
{
	...
	public class QueryTests
	{
		...
		{
			QueryBuilder queryBuilder = new QueryBuilder();
			queryBuilder.Setup(x => x.Field("display_name", "a*"), x => x.Language(Language.Parse("en")));
			...
			sitecoreItemResult.Items.ForEach(
				item =>
					{
						Console.WriteLine(item.Name);
						Assert.IsTrue(item["Display Name"].IndexOf("a", StringComparison.InvariantCultureIgnoreCase) >= 0);
					});
			Assert.Greater(sitecoreItemResult.Items.Count, 0);
		}
